...
command(
  ...
  async (message, match) => {
    ...
    if (isban) {
      return await message.sendMessage(message.jid, "Bot is already banned you");
    }
    await banUser(chatid);
    return await message.sendMessage(message.jid, "you are banned for user bot");
  },
);
...
command(
  ...
  async (message, match) => {
    ...
    const isban = await isBanned(chatid);
    if (!isban) {
      return await message.sendMessage(message.jid, "Bot is not banned you");
    }
...
command(
  ...
  async (message, match) => {
    ...
    if (!isAdmin(message.jid, message.user, message.client))
      return await message.reply("_I'm not admin_");
    await message.reply(
      "_This action will remove all non-admin participants from the group. Are you sure? Reply with 'yes' to confirm and i'll start my vawulence._",
    );
